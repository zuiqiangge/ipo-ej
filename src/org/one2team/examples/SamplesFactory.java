...
public class SamplesFactory {
	...
	public ChartOptions createColumnBasic() {
		...
		try {
			chartOptions = factory.createChartOptions ();
	...
	public ChartOptions createColumnBasic(TbXtNewsService tbXtNewsService,Map<String,Object> params) {
		// http://highcharts.com/demo/column-basic
		ChartOptions chartOptions = null;
		try {
			chartOptions = factory.createChartOptions ();
			...
			// titles
			chartOptions.getTitle ().setText ("全球财经统计图");
//			chartOptions.getSubtitle ().setText ("Source: WorldClimate.com");
			
			params.put("rownum", 4);
			List<News> newsList = tbXtNewsService.yqrdListByRownum(params);
			
			double[] dbs = new double[4];
			int i = 0;
			for (News news : newsList) {
				// xAxis
//				chartOptions.getXAxis ().getCategories ().pushString(HanLPUtil.extractKeyword(news.getTitle()));
				String title = news.getTitle();
				if(title.length() > 10){
					title = title.substring(0,10);
				}
				chartOptions.getXAxis ().getCategories ().pushString(title);
				dbs[i] = Double.parseDouble(news.getClickedCounts());
				i++;
			}
			// yAxis
//			chartOptions.getYAxis ().setMin (0).getTitle ().setText ("Rainfall (mm)");
