	...
	$(".user_pic").find(".UserPrompt").css("display", "block");
			...
			<div class="page-container2">
				...
				<div class="content eventAnalysis rel" id="wrap">
					...
					<li	class="current" onclick="toggleTab(1,this,'<%=basePath%>taskIndex_taskList1?type=${tbXtTask.type}');">
						用户列表</li>
						</ul>
						<div class="tab_box">
...
$("#tickets").val("");
									...
									<c:forEach items="${tbXtYhxx.results}" var="user" varStatus="No">
										<tr id="${user.id}">
											<td>
												${No.index+1}
											</td>
                                        ...
                                        <td  class="link">
                                            <a href="#" onclick="goCreate('<%=basePath%>main/home/usercpm.jsp?loginId=${user.loginId}&id=${user.id}');">修改密码</a> |
                                            <a href="javascript:void(0);" onclick="javascript:del('${user.loginId}');">删除</a>
                                        </td>
										</tr>
									</c:forEach>
											...
											<td colspan="15" class="page_info"
												...
												<div style="float: right; padding-right: 0px; width: auto;">
													<div id="Pagination">
														<ul class="pagination">
															<c:choose>
																<c:when test="${tbXtYhxx.pageNo==1 }"><li class="first disabled"><a href="#">首页</a></li></c:when>
																<c:otherwise><li class="first active"><a href="<%=basePath%>home_queryUser?pageNo=1">首页</a></li></c:otherwise>
															</c:choose>
															<c:choose>
																<c:when test="${tbXtYhxx.pageNo==1 }"><li class="prev disabled"><a href="#">上一页</a></li></c:when>
																<c:otherwise><li class="prev"><a href="<%=basePath%>home_queryUser?pageNo=${tbXtTask.pageNo-1}">上一页</a></li></c:otherwise>
															</c:choose>
															<c:forEach begin="${tbXtYhxx.startPage}" end="${tbXtYhxx.endPage}" var="No">
																<c:choose>
																<c:when test="${tbXtYhxx.pageNo==No }"><li class="page active"><a href="#">${No}</a></li></c:when>
																<c:otherwise><li class="page"><a href="<%=basePath%>home_queryUser?pageNo=${No}">${No}</a></li></c:otherwise>
																</c:choose>
															</c:forEach>
															<c:choose>
																<c:when test="${tbXtYhxx.pageNo==tbXtYhxx.totalPage }">
																	<li class="next disabled"><a href="#">下一页</a></li>
																	<li class="next disabled"><a href="#">末页</a></li>
																</c:when>
																<c:otherwise>
																<li class="next"><a href="<%=basePath%>home_queryUser?pageNo=${tbXtYhxx.pageNo+1}">下一页</a></li>
																<li class="next"><a href="<%=basePath%>home_queryUser?pageNo=${tbXtYhxx.totalPage}">末页</a></li>
																</c:otherwise>
															</c:choose>
														</ul>
													</div>
