...
<script type="text/javascript">
	...
	//通过地区id获得地区关键词
	function getkeywordsbyssdq(ssdq){
		$.ajax({
			 type: "POST",
		     url: "<%=basePath%>regionkeyword_findKeywordBySsdq.action",
		     dataType: "json",
		     data:{"keyword.ssdq":ssdq},
			...
			<div class="page-container2">
				...
				<div class="content eventAnalysis rel" id="wrap">
					...
					<div class="frameCon pading10">
						...
						<div class="tab_box">
							<form action="<%=basePath%>regionkeyword_update" method="post" id="keyform">
								<p style="text-align: center; padding-left: 50px; padding-bottom: 20px;padding-top: 10px;">
								<span>
									选择地区：
								</span>
								<select class="inputDH " onchange="getkeywordsbyssdq(this.value);" id="ssdq" name="keyword.ssdq" style="width: 507px;" name="modelid" cols="67" rows="3">
									<s:iterator id="region" value="result">
										<s:if test="%{keyword != null && keyword.ssdq == #region.CODE}">
											<option selected="selected" value="<s:property value="#region.CODE"/>"><s:property value="#region.DEFINE"/></option>
										</s:if>
										<s:else>
											<option value="<s:property value="#region.CODE"/>"><s:property value="#region.DEFINE"/></option>
										</s:else>
									</s:iterator>
								</select>
								</p>
								...
								<textarea cols="100" rows="8" id="keywords" name="keyword.keywords" style="border: 1 solid #888888;LINE-HEIGHT:18px;padding: 3px;"></textarea>
								</p>
								<p style="text-align: center; padding: 10px;">
									<a class="button button2 ml10" href="javascript:submitfrom();">提交</a>
								</p>
							</form>
						</div>
